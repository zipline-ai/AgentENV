...
Plan for review, based on fork main `8cda52c`; no implementation or production
changes. Keep `TimeoutStopSec=30`, unit/package settings and HTTP drain unchanged.
...
The stop returned after 41.608 s, including the existing stop helper, with five
Firecracker processes left. The direct-SIGTERM control completed all three calls
and the final 600 ms backoff: reporter ended at +31.206 s, guest preservation
then took 502 ms, total exit 31.883 s. Restart lost the running guest's record.
...
unregister. Two seconds is best-effort telemetry within the five-second HTTP
drain, leaving the guest path the full stop window. Log monotonic phase times,
remaining operations and unregister outcome. A missed unregister leaves stale
...
or rollback. Crash retention prevents destruction; automatic recovery requires
exact-runtime reconciliation and is not claimed by this minimal fix. Neither
reordering nor retention proves bounded completion of arbitrary hung storage.
...
no backend launch/route/GC deletion). Keep healthy-reporter, repeated-signal,
HTTP-error and cleanup-held controls; prove no surviving reporter tasks. Run
proxy/orchestrator/persistence tests, fmt and clippy. Mock bounds are not KVM proof.
...
helper kills/unlinks after ~10 seconds. No bypass of that helper or changed
cleanup budget to manufacture a pass. Archive every result and delete resources.
Rollout stays blocked on successful real cleanup/recovery evidence and unresolved
storage/Resuming recovery; no whole-node guarantee follows from telemetry alone.
